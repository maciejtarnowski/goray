...
func main() {
	...
	w := world.NewWorld()
	w.Objects = append(w.Objects, getFloor(bgMaterial), getLeftWall(bgMaterial), getRightWall(bgMaterial), getMiddleSphere(), getRightSphere(), getLeftSphere())
...
func getFloor(m *material.Material) ray.Object {
	...
	floor.SetTransformation(tr.NewScaling(10, 0.01, 10))
	floor.SetMaterial(m)

	return floor
}

func getLeftWall(m *material.Material) ray.Object {
	leftWall := shape.NewSphere()
	leftWall.SetTransformation(
		tr.NewTranslation(0, 0, 5).MultiplyMatrix(tr.NewRotationY(-math.Pi / 4)).MultiplyMatrix(tr.NewRotationX(math.Pi / 2)).MultiplyMatrix(tr.NewScaling(10, 0.01, 10)),
	)
...
func getRightWall(m *material.Material) ray.Object {
